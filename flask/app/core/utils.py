...
def stringify_given_datetime_or_current_datetime(
    datetime_=None, format_="%Y-%m-%d %H:%M:%S"
):
    """
...
def parse_given_str_datetime_or_current_datetime(
    datetime_=None, format_="%Y-%m-%dT%H:%M"
):
    try:
        try:
...
def verify_password(password, salt, hashed_password):
    ...
    if (
        hashed_password
        == hashlib.sha512(password_with_salt.encode("utf-8")).hexdigest()
    ):
        return True
    else:
        False
